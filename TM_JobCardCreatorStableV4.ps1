...
$XPPS1 = 1450
Clear-Host
Write-Host "Enter quote number to pull from quote`nType 'QHC' to pull from standard parts list`nType 'Custom' to enter custom job`nType 'Load' to load last custom job" -ForegroundColor White
$QN = read-host "Sudo"
if($QN -match "2015"){
...
if(!$checkbox){
    ...
    if($QN -eq "QHC"){
        $Quotesheet = get-item "$ToolShop\05. Share\11. Tool Shop Quotes\01_New-parts\$QuoteFolder\$QN*.xlsm"
    }
    else{
        $QN = $QN.Trim("Q")
